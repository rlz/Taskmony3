...
//             new Mock<ICommentRepository>().Object, new Mock<IRecurringTaskGenerator>().Object);
//     }
//
//     [Fact]
...
//
//         Assert.IsAssignableFrom<List<Models.Tasks.Task>>(result);
...
//                 It.Is<IEnumerable<Models.Tasks.Task>>(t => t.All(x => x.GroupId == task.GroupId))), Times.Once);
//         _mockTasksRepository.Verify(r => r.SaveChangesAsync(), Times.Once);
//     }
//
//     [Fact]
...
//         _mockTasksRepository.Verify(r => r.AddAsync(It.IsAny<Models.Tasks.Task>()), Times.Never);
//     }
//
//     [Fact]
...
//             .Verify(r => r.AnyMemberWithIdAsync(task.DirectionId!.Value, task.CreatedById), Times.Once);
//         _mockTasksRepository.Verify(r => r.SaveChangesAsync(), Times.Never);
//         _mockTasksRepository.Verify(r => r.AddRangeAsync(It.IsAny<IEnumerable<Models.Tasks.Task>>()), Times.Never);
//     }
//
//     [Fact]
...
//             .Verify(r => r.AnyMemberWithIdAsync(task.DirectionId!.Value, task.CreatedById), Times.Once);
//         _mockTasksRepository
//             .Verify(r => r.AddRangeAsync(It.Is<IEnumerable<Models.Tasks.Task>>(
//                     t => t.All(x =>
...
//             Times.Once);
//         _mockTasksRepository.Verify(r => r.SaveChangesAsync(), Times.Once);
//         _mockTasksRepository.Verify(r => r.AddRangeAsync(It.Is<IEnumerable<Models.Tasks.Task>>(
//             t => t.All(x =>
...
//     }
//
//     private async Task TestTaskSetterUpdateFailsAsync<T>(Models.Tasks.Task task, Guid userId, T value,
...
//     }
//
//     private async Task TestTaskSetterNoAccessAsync<T>(Models.Tasks.Task task, Guid userId, T value,
//         Func<Guid, T, Guid, Task<Guid?>> setter)
//     {
//         _mockTasksRepository.Setup(r => r.GetByIdAsync(task.Id)).ReturnsAsync(task);
//
...
//     }
//
//     private async Task TestRecurringTaskSetterNoAccessAsync<T>(List<Models.Tasks.Task> tasks, Guid groupId,
...
//     }
//
//     private async Task TestTaskSetterNotFoundAsync<T>(Models.Tasks.Task task, Guid userId, T value,
//         Func<Guid, T, Guid, Task<Guid?>> setter)
//     {
//         _mockTasksRepository.Setup(r => r.GetByIdAsync(task.Id)).ReturnsAsync((Models.Tasks.Task?) null);
//
//         var exception = await Assert.ThrowsAsync<DomainException>(() => setter(task.Id, value, userId));
//
...
//     }
//
//     private async Task TestRecurringTaskSetterTaskNotFoundAsync<T>(List<Models.Tasks.Task> tasks, Guid groupId,
//         Guid userId, T value, Func<Guid, Guid, T, Guid, Task<IEnumerable<Guid>>> setter)
//     {
//         _mockTasksRepository.Setup(r => r.GetActiveTasksAsync(groupId)).ReturnsAsync(tasks);
//
...
//     }
//
//     private async Task TestRecurringTaskSetterGroupNotFoundAsync<T>(List<Models.Tasks.Task> tasks, Guid userId, T value,
//         Func<Guid, Guid, T, Guid, Task<IEnumerable<Guid>>> setter)
//     {
//         var unknownGroupId = Guid.NewGuid();
//         _mockTasksRepository.Setup(r => r.GetActiveTasksAsync(unknownGroupId)).ReturnsAsync(Array.Empty<Models.Tasks.Task>);
//
//         var exception =
...
//     }
//
//     private async Task TestRecurringTaskSetterUpdateFailsAsync<T>(List<Models.Tasks.Task> tasks, List<Models.Tasks.Task> notActive,
...
//     }
//
//     private async Task TestTaskSetterValidationFailsAsync<T>(Models.Tasks.Task task, Guid userId, T value,
...
//     }
//
//     private async Task TestRecurringTaskSetterValidationFailsAsync<T>(List<Models.Tasks.Task> tasks, Guid groupId,
...
//     }
//
//     private async Task TestTaskSetterAsync<T>(Models.Tasks.Task task, Guid userId, T value, Func<Models.Tasks.Task, T> getter,
//         Func<Guid, T, Guid, Task<Guid?>> setter)
//     {
//         _mockTasksRepository.Setup(r => r.SaveChangesAsync()).ReturnsAsync(true);
//         _mockTasksRepository.Setup(r => r.GetByIdAsync(task.Id)).ReturnsAsync(task);
...
//     }
//
//     private async Task TestRecurringTaskSetterAsync<T>(List<Models.Tasks.Task> tasks, Guid groupId, Guid userId, T value,
//         Func<Models.Tasks.Task, T> getter, Func<Guid, Guid, T, Guid, Task<IEnumerable<Guid>>> setter)
//     {
//         _mockTasksRepository.Setup(r => r.SaveChangesAsync()).ReturnsAsync(true);
//         _mockTasksRepository.Setup(r => r.GetActiveTasksAsync(groupId)).ReturnsAsync(tasks);
//
...
//     }
//
//     private static bool IsVisibleToUser(Models.Tasks.Task task, Guid userId)
//     {
