...
export const NotificationsBtn = ({ onClick }: NotificationsBtnPropsT) => {
  ...
  return (
    <div className={"absolute top-0 right-0 m-4 flex gap-2"}>
      {isSmallScreen && 
      <img src={filter} className={"block m-0 width-50"} alt=""
      onClick={changeFilterWindow}/>
      
      }
      <div onClick={(e) => onClick()}><button
        data-after-text={digit}
        data-after-type="badge top right"
        type="button"
      >
        <img src={isSmallScreen?notificationsMobile:notifications} className={"block m-0 width-50"} alt=""/>
      </button>
      </div>
    </div>
  );
