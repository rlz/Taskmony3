...
import {
  EditedTask,
  OpenTask,
} from "../../components/task-idea/task/open-task/open-task";
import {
  addRepeatedTasks,
  addTask,
...
import { FilterByFuture } from "../../components/other-components/filter/by-task-type";
...
function Tasks({ directionId, directionName }: TasksProps) {
  ...
  let tasksToShow = useAppSelector((store) => store.tasks.items)
    .filter((t) => t.deletedAt == null && t.direction?.id === directionId)
    .sort((a, b) => {
  ...
  if (assignedTo.length > 0) {
    tasksToShow = tasksToShow.filter((i) =>
      assignedTo.includes(i.assignee?.id)
    );
  }
  if (!future || future === "0") {
