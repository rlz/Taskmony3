...
export const CHANGE_TASK_GROUP_ID = "CHANGE_TASK_GROUP_ID";
...
export const CHANGE_TASKS_DESCRIPTION_SUCCESS =
...
export const CHANGE_TASK_REPEAT_MODE_FROM_REPEATED_SUCCESS =
  "CHANGE_TASK_REPEAT_MODE_FROM_REPEATED_SUCCESS";
    ...
    getAccessToken()
      .then((cookie) =>
        fetch(URL, {
          ...
          body: JSON.stringify({
            query: `mutation {
      taskSetDescription(taskId:"${taskId}",${
    ...
    `,
      ...
      .then((res) => {
        if (res) {
          if (groupId) {
            dispatch({
              type: CHANGE_TASKS_DESCRIPTION_SUCCESS,
              groupId: groupId,
              payload: description,
            });
          } else {
...
export function changeTaskDetails(
  taskId: string,
  details: string | null,
  groupId?: string
) {
    ...
    `,
      ...
      .then((res) => {
        if (res) {
          if (groupId) {
            dispatch({
              type: CHANGE_TASKS_DETAILS_SUCCESS,
              groupId: groupId,
              payload: details,
            });
          } else {
...
export function changeTaskDirection(
  taskId: string,
    ...
    getAccessToken()
      .then((cookie) =>
        fetch(URL, {
          ...
          body: JSON.stringify({
            query: `mutation {
      taskSetDirection(taskId:"${taskId}",${
              groupId ? `groupId:"${groupId}",` : ""
    ...
    `,
      ...
      .then((res) => {
        if (res) {
          if (groupId) {
            dispatch({
              type: CHANGE_TASKS_DIRECTION_SUCCESS,
              groupId: groupId,
              payload: direction,
            });
          } else {
    ...
    `,
      ...
      .then((res) => {
        if (res) {
          if (groupId) {
            dispatch({
              type: CHANGE_TASKS_ASSIGNEE_SUCCESS,
              groupId: groupId,
              payload: assignee,
            });
          } else {
    ...
    `,
      ...
      .then(async (res) => {
        ...
        if (res) {
          ...
          //download new
          const newIds = res.data.taskSetRepeatMode;
          const newTasks = await getTasksById(newIds);
          //change edited task
          dispatch({
            type: CHANGE_TASK,
            task: newTasks[0],
          ...
          dispatch({
            type: CHANGE_OPEN_TASK,
            task: newTasks[0],
          });
          //add the rest of tasks
          dispatch({
            type: ADD_TASKS,
            items: newTasks.splice(1, newTasks.length),
          });
        } else {
          dispatch({
...
export function changeTaskRepeatUntil(
  taskId: string,
  repeatUntil: string,
  groupId?: string
) {
...
export function deleteTask(id: string, groupId?: string) {
  const date = nowDate();
  return function (dispatch: Dispatch) {
